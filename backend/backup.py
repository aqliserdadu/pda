...
# === Path Konfigurasi ===
BACKUP_DIR = "/opt/pda/database/backup"
STATE_FILE = "/opt/pda/database/backup_state.json"
...
def backup_database():
    ...
    sql_filename = f"pda_db_{today_str}.sql"
...
def cleanup_old_backups():
    ...
    for fname in os.listdir(BACKUP_DIR):
        if fname.startswith("pda_db_") and fname.endswith(".sql.gz"):
            try:
                date_str = fname.replace("pda_db_", "").replace(".sql.gz", "")
